...
if __name__ == "__main__":
    ...
    noise_model_fn = meta_info["noise_model_fn"]
    backend_config_fn = meta_info["backend_config_fn"]
    ...
    noise_model = noise_model_fn if noise_model_fn is not None else None
    backend_config = backend_config_fn if backend_config_fn is not None else None
    ...
    for instance_init_meas in rank_jobs:
        ...
        elif eval_mode == "qasm_noisy":
            subcircuit_inst_prob = evaluate_circ(
                circuit=subcircuit_instance,
                backend="qasm_simulator",
                options={"num_shots": num_shots, "noise_model": noise_model, "backend_config": backend_config, "mitigated": False},
            )
        elif eval_mode == "qasm_noisy_mitigated":
            subcircuit_inst_prob = evaluate_circ(
                circuit=subcircuit_instance,
                backend="qasm_simulator",
                options={"num_shots": num_shots, "noise_model": noise_model, "backend_config": backend_config, "mitigated": True},
            )
        else:
